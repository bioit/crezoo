...
--
-- $Log$
-- Revision 1.2  2005/02/17 16:18:58  heto
...
CREATE OR REPLACE VIEW V_ROLE_PRIV_1 ( 
  RID, PRID, NAME, COMM, INCL) AS
    select distinct ro.rid, p.prid, p.name, p.comm,
	(SELECT (case when count(prid) = 0 then 0 else 1 end) from r_rol_pri where rid=r.rid and prid=p.prid)
    from Roles_ ro right join r_rol_pri r 
                on ro.rid = r.rid
        , privileges_ p
  order by prid;
...
create or replace view V_PROJECTS_2 (SID, SNAME, PID, NAME, 
		...
		select distinct s.sid, s.name, p.pid, p.name,
			   			p.comm, p.status 
		from projects p 
			right join r_prj_spc r on r.pid = p.pid
			right join species s on s.sid = r.sid;
...
CREATE OR REPLACE VIEW V_PROJECTS_3 (
  ...
  select r1.sid, r2.suid, r3.id,
  		 p.pid, p.name,	p.comm, p.status
  from	projects p 
		left join r_prj_spc r1 on p.pid = r1.pid
		left join r_prj_su r2 on p.pid = r2.pid
		left join r_prj_rol r3 on p.pid = r3.pid;
...
create or replace view V_Sampling_Units_3 (
  ...
  select r.pid, su.sid, s.name,  su.suid, su.name, su.comm, 
	  	 su.status, sui.inds, u.usr, su.ts
  from R_Prj_SU r, Sampling_Units su left join v_sampling_units_inds sui on su.suid = sui.suid  , Species s, 
  	   Users u
  where
  	   r.suid = su.suid and
	   u.id = su.id and
...
create or replace view V_Individuals_2 as
  select i.suid, i.iid, i.identity, i.alias, i.sex, i.birth_date, i.comm,
         --decode(i.father, null, null, decode(f.status, 'E', f.identity, '(' || f.identity || ')')) as fidentity,
         (select (case when i.father is null then null else (select case when f.status = 'E' then f.identity else '(' || f.identity || ')' end) end)) as fidentity,
		 
         --decode(i.mother, null, null, decode(m.status, 'E', m.identity, '(' || m.identity || ')')) as midentity, 
         (select (case when i.mother is null then null else (select case when m.status = 'E' then m.identity else '(' || m.identity || ')' end) end)) as midentity,
...
create or replace view V_Individuals_3 as
  select s.name as suname, i.suid, i.iid, i.identity, i.alias, i.sex, i.birth_date, i.comm,
         (select (case when i.father is null then null else (select case when f.status = 'E' then f.identity else '(' || f.identity || ')' end) end)) as fidentity,
         --decode(i.father, null, null, decode(f.status, 'E', f.identity, '(' || f.identity || ')')) as fidentity,
         (select (case when i.mother is null then null else (select case when m.status = 'E' then m.identity else '(' || m.identity || ')' end) end)) as midentity,
         --decode(i.mother, null, null, decode(m.status, 'E', m.identity, '(' || m.identity || ')')) as midentity, 
...
--  select r.pid,
--         su.suid, su.name as suname,
--         i.iid, i.identity, i.alias, i.sex, i.birth_date, i.comm,
--         f.identity as fidentity, m.identity as midentity,
--         u.usr, i.ts
--  from  R_Prj_SU r, 
--        Sampling_Units su,
--        Individuals i, 
...
create or replace view V_Individuals_Log as
  select l.iid, l.identity, l.alias, l.sex, l.birth_date, l.comm,
         ...
         u.usr, l.ts, l.status
  from  Individuals_Log l 
            left join Individuals f on l.father = f.iid 
            left join Individuals m on l.mother = m.iid,
        Users u
  where l.id = u.id;
...
create or replace view V_Groupings_Grps as
  select gsid, count(gid) as grps
  from Groups group by gsid;

--create or replace view V_Groupings_2 as
--  select r.pid,
--         v_su.suid, v_su.name as suname,
--         gs.gsid, gs.name as name, gs.comm,
--         gg.grps,
--         u.usr, gs.ts
--  from  R_Prj_SU r, 
...
create or replace view V_Groupings_2 as
  ...
  from  R_Prj_SU r, 
        v_enabled_sampling_units_1 v_su, 
        Groupings gs left join V_Groupings_Grps gg on gs.gsid = gg.gsid,
        Users u
  where gs.suid = r.suid
    and gs.suid = v_su.suid
    and gs.id = u.id;
...
create or replace view V_Groups_Inds as
  select r.gid, count(r.gid) as inds
...
create or replace view V_Groups_3 as
  ...
  from  R_Prj_SU r, 
        v_enabled_sampling_units_1 v_su, 
        Groupings gs, 
        Groups g left join V_Groups_Inds gi on g.gid = gi.gid,
        Users u
  where gs.suid = r.suid
    and gs.suid = v_su.suid
    and g.gsid = gs.gsid
...
create or replace view V_Variable_Sets_3 as
  select r.pid, v_su.suid, v_su.name as suname,
...
create or replace view V_Positions_1 (
  ...
  select m.mid, m.name, m.position, p.value,
  		 --decode(p.value, NULL, m.position, p.value),
                 (select case when p.value is null then m.position else p.value end),
		 ms.msid, ms.name
  from Markers m, Positions p, Marker_Sets ms, v_enabled_sampling_units_1 v_su
  where
  	   m.mid = p.mid and
...
create or replace view V_Genotypes_4 (
	   ...
	   IID, IDENTITY, AID1, AID2, A1NAME, A2NAME,
	   RAW1, RAW2, LEVEL_, REFERENCE, COMM, USR, TS) as
  select r.pid, s.sid, s.name, c.cid, c.name as cname,
         m.mid, m.name as mname,
         v_su.suid, v_su.name as suname,
         v_i.iid, v_i.identity,
		 gt.aid1, gt.aid2, a1.name, a2.name ,
...
--         a1.name as a1name, a2.name as a2name, g.reference, 
...
RAW1, RAW2, COMM ) AS select m.mid, m.name, 
  ...
  from markers m, 
       individuals i, 
       genotypes g 
            left join alleles a1 on g.aid1 = a1.aid
            left join alleles a2 on g.aid2 = a2.aid
  where 
	   i.status = 'E' and 
           i.iid = g.iid and 
           m.mid = g.mid;
...
create or replace view V_Genotypes_Log (
  ...
  select gtl.mid, gtl.iid,
	 gtl.aid1, gtl.aid2, a1.name, a2.name,
         ...
         u.usr, gtl.ts
  from  Genotypes_Log gtl 
            left join Alleles a1 on gtl.aid1 = a1.aid
            left join Alleles a2 on gtl.aid2 = a2.aid,
        Users u
  where 
        gtl.id = u.id;
...
CREATE OR REPLACE VIEW V_R_UMID_MID_1 ( 
  PID, SUID, UMID, MID, TS) AS
    select 
        um.pid, m.suid, r.umid, r.mid, r.ts 
    from 
        r_umid_mid r,
        markers m,
        u_markers um,
...
CREATE OR REPLACE VIEW V_R_UMID_MID_2 ( 
  PID, SUID, SUNAME, UMID, MID, MNAME, TS ) AS
    select 
        um.pid, m.suid, v_su.name, r.umid, r.mid, m.name, r.ts 
    from 
        r_umid_mid r, 
        markers m,
        u_markers um,
...
CREATE OR REPLACE VIEW V_R_UAID_AID_1 ( 
  PID, UMID, AID, UAID, TS) AS
    select 
        um.pid, um.umid, r.aid, r.uaid, r.ts 
    from 
        r_uaid_aid r, 
        markers m, 
        alleles a,
...
create or replace view V_U_Positions_1 (
	   UMID, UMNAME, DEF_POSITION, OVER_POSITION, POSITION, UMSID, UMSNAME) as
  select um.umid, um.name, um.position, up.value,
  		 (select case when up.value is null then um.position else up.value end),
                 --decode(up.value, NULL, um.position, up.value),
		 ums.umsid, ums.name
  from U_Markers um, U_Positions up, U_Marker_Sets ums
  where
...
CREATE OR REPLACE VIEW V_R_UVID_VID_1 ( 
    ...
    select uv.pid, v.suid, r.uvid, r.vid, v.name, s.name, r.ts    
    from 
        r_uvid_vid r, 
        variables v,
        U_VARIABLES uv,
...
create or replace view V_FILE_GENERATIONS_3 (
  ...
  select a.pid, a.fgid, a.name, a.mode_,
  		 a.type_, a.msid,
		 (select case when mode_ = 'S' 
                    then 
                        (select name from marker_sets where msid=a.msid) 
                    else 
                        (select name from u_marker_sets where pid=a.pid and umsid=a.msid) 
                    end),
                 --decode(mode_, 'S', (select name from marker_sets where msid=a.msid),
		 ...
		 a.vsid,
                 (select case when mode_ = 'S'
                    then
                        (select name from variable_sets where vsid=a.vsid)
                    else
                        (select name from u_variable_sets where pid=a.pid and uvsid=a.vsid)
                    end),
		 --decode(mode_, 'S', (select name from variable_sets where vsid=a.vsid),
