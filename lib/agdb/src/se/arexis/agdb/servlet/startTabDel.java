/*
  $Log$
  Revision 1.11  2005/04/04 13:58:09  heto
  Commit before merging ant-scripts

  Revision 1.10  2005/03/24 15:12:44  heto
  Working with removing oracle dep.

  Revision 1.9  2005/03/22 12:50:17  heto
  Working with moving all export files to a new package: se.arexis.agdb.util.FileExport

  Revision 1.8  2005/03/03 15:41:37  heto
  Converting for using PostgreSQL

  Revision 1.7  2004/03/30 14:21:22  wali
  Changed Analyses to export.

  Revision 1.6  2003/12/19 15:33:12  wali
  ...
  Revision 1.3  2002/12/13 15:03:30  heto
  Moved function to ArexisServlet

  Revision 1.2  2002/11/13 09:06:48  heto
  ...
  Revision 1.10  2002/01/29 18:03:10  roca
  Changes by roca (se funktionsbskrivnig for LF025)

  Revision 1.9  2001/06/26 12:08:19  roca
...
import se.arexis.agdb.util.FileExport.GenGTF;

public class startTabDel extends SecureArexisServlet
{
   ...
   private void writeStep1(HttpServletRequest req, HttpServletResponse res)
      ...
      if(rset.next())
      {
            ...
            writeErrorPage(req, res, "Generations.Start.GTF", "The name you have choosen already exists!",
                           "viewFile/start" + "?");
   ...
   private void writeStep1Multi(HttpServletRequest req, HttpServletResponse res)
      ...
      try {
         ...
         out.println("<center>" +
                     "<b style=\"font-size: 15pt\">Export file - Start - Tab delimited 1(3)</b></center>");
         out.println("</td></tr>");
         out.println("<tr><td width=\"736\" colspan=\"2\" height=\"2\" bgcolor=\"#008B8B\">&nbsp;</td>");
         out.println("</tr></table>");
         out.println("<form method=post action=\"" +
                     getServletPath("startTabDel/step1") + "\">");
         /*
           out.println("<form method=post action=\"" +
   ...
   private void writeStep2Multi(HttpServletRequest req, HttpServletResponse res)
      ...
      try {
         ...
         out.println("<td width=736 colspan=2 height=15>");
         out.println("<center>" +
                     "<b style=\"font-size: 15pt\">Analyses - File Generation (multi) - Start - GTF 2(3)</b></center>");
         out.println("</td></tr>");
         out.println("<tr><td width=\"736\" colspan=\"2\" height=\"2\" bgcolor=\"#008B8B\">&nbsp;</td>");
         out.println("</tr></table>");
         out.println("<form method=post action=\"" +
                     //  getServletPath("startTabDel/step3Single") + "\">");
                     getServletPath("startTabDel/step3Multi") + "\">");
         ...
         out.println("<tr>");
         out.println("<td width=200>Type</td>");
         out.println("<td width=300>General Table Format</td>");
         out.println("</tr>");
         out.println("<tr>");
         out.println("<td width=200>Mode</td>");
         out.println("<td width=300>Multiple Sampling Units</td>");
   ...
   private void writeStep3Multi(HttpServletRequest req, HttpServletResponse res)
      ...
      try {
         ...
         out.println("<td width=736 colspan=2 height=15>");
         out.println("<center>" +
                     "<b style=\"font-size: 15pt\">Analyses - File Generation (multi) - Start - GTF 3(3)</b></center>");
         out.println("</td></tr>");
         out.println("<tr><td width=\"736\" colspan=\"2\" height=\"2\" bgcolor=\"#008B8B\">&nbsp;</td>");
         out.println("</tr></table>");
         out.println("<form method=post action=\"" +
                     getServletPath("startTabDel/startMulti") + "\">");
         ...
         out.println("<tr><td><input type=checkbox name=ref checked></td><td>Reference</td>");
   ...
   private void writeStep1Single(HttpServletRequest req, HttpServletResponse res)
      ...
      try {
         ...
         out.println("<td width=736 colspan=2 height=15>");
         out.println("<center>" +
                     "<b style=\"font-size: 15pt\">Export file - Start - GTF 1(3)</b></center>");
         out.println("</td></tr>");
         out.println("<tr><td width=\"736\" colspan=\"2\" height=\"2\" bgcolor=\"#008B8B\">&nbsp;</td>");
         out.println("</tr></table>");
         out.println("<form method=post action=\"" +
                     getServletPath("startTabDel/step2Single") + "\">");
   ...
   private void writeStep2Single(HttpServletRequest req, HttpServletResponse res)
      ...
      try {
         ...
         out.println("<td width=736 colspan=2 height=15>");
         out.println("<center>" +
                     "<b style=\"font-size: 15pt\">Export file - Start - GTF 2(3)</b></center>");
         out.println("</td></tr>");
         out.println("<tr><td width=\"736\" colspan=\"2\" height=\"2\" bgcolor=\"#008B8B\">&nbsp;</td>");
         out.println("</tr></table>");
         out.println("<form method=post action=\"" +
   ...
   private void writeStep3Single(HttpServletRequest req, HttpServletResponse res)
      ...
      try {
         ...
         out.println("<td width=736 colspan=2 height=15>");
         out.println("<center>" +
                     "<b style=\"font-size: 15pt\">Export file - Start - GTF 3(3)</b></center>");
         out.println("</td></tr>");
         out.println("<tr><td width=\"736\" colspan=\"2\" height=\"2\" bgcolor=\"#008B8B\">&nbsp;</td>");
         out.println("</tr></table>");
         out.println("<form method=post action=\"" +
                     getServletPath("startTabDel/startSingle") + "\">");
         ...
         out.println("<tr><td><input type=checkbox name=ref checked></td><td>Reference</td></tr>");
   ...
   void startSingle(HttpServletRequest req, HttpServletResponse res)
      ...
      try {
         ...
         // String nullRepl = getNullReplacement(session);
         ...
         */
         if (dburl == null || uid == null || pwd == null || absPath == null) {
            // System.err.println("Unable to read initialization parameters needed by startTabDel");
            ok = false;
            writeErrorPage(req, res, "Generations.Start.GTF",
                           Errors.keyValue("Generations.Start.GTF." +
                                           "ReadInitParams.Error.Msg"),
                           "viewFile/start");
         } else {
            // Create the neccesary databas objects!
             
             DbFileGeneration fg = new DbFileGeneration();
             //fgid = fg.CreateSingleFileGeneration(conn, msid, vsid, gsid, "General Table Format", name, comm, pid);
             String type = "General Table Format";
             
             fgid = fg.CreateSingleFileGeneration(conn, msid, vsid, gsid, 
                     type, name, comm, Integer.valueOf(pid).intValue(), 
                     Integer.valueOf(suid).intValue(), 
                     Integer.valueOf(fid).intValue(), 
           ...
           /*
             ...
             if (gsid==null)
                 ...
                 tmp = Integer.valueOf(gsid);
            
            fg.Create_FG_FLT_Link(conn, fgid, Integer.valueOf(suid).intValue(), 
                    Integer.valueOf(fid).intValue(), tmp);
            tmp = null;
            ...
            */
            ...
            if (!createPath(absPath)) {
               writeErrorPage(req, res, "Generations.Start.GTF",
                              Errors.keyValue("Generations.Start.GTF.CreateDir.Error.Msg"),
                              "viewFile/start");
               ok = false;
               throw new Exception();
            }
            gtf = new GenGTF(fgid, absPath, dbdriver, dburl, uid, pwd, nullRepl, field_mask, includeRef,includeRaw);
  				// No rush. We don't want to overload the server with time consuming tasks

            gtf.setPriority(Thread.NORM_PRIORITY - 2);
            gtf.start();
         }
      ...
      try {
         ...
         */
         if (dburl == null || uid == null || pwd == null || absPath == null) {
            ...
            cstmt.setString(4, "General Table Format");
            if (umsid == null)
            ...
            */
            ...
            for (int i=0; i < suids.size(); i++)
            {
               ...
               if (message != null && !message.trim().equals("")) {
                  ok = false;
                  writeErrorPage(req, res, "Generations.Start.GTF", message,
                                 "viewFile/start");
                  throw new Exception();
               }
            ...
            gtf = new GenGTF(fgid, absPath, dbdriver, dburl, uid, pwd, nullRepl, field_mask, includeRaw, includeRef);
  				// No rush. We don't want to overload the server with time consuming tasks
